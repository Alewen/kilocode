...
int main(int argc, char* argv[])
{
    ...
    RPC_IF_HANDLE ifSpec = CallbackDemo_v1_0_s_ifspec;
    RPC_BINDING_VECTOR* pBindingVec = NULL;

    printf("RPC 上下文句柄服务端启动中...\n");
    ...
    status = RpcEpRegister(ifSpec, pBindingVec, NULL, (unsigned char*)"CallbackServer");
    if (status) { printf("RpcEpRegister 失败: 0x%x\n", status); return 1; }

    printf("服务端正在监听 ncalrpc:[CallbackDemo]\n");

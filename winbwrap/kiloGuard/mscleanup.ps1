...
$fileExists = Test-Path "$env:SystemRoot\System32\drivers\$name.sys"
...
# Step 2: Unload minifilter
Write-Host " [2] Unloading minifilter ..."
if ($driverInstalled) {
    $flt = fltmc filters 2>&1 | Select-String -Pattern $name -SimpleMatch
    if ($flt) {
...
if ($driverInstalled) {
    if (Test-Path "HKLM:\SYSTEM\CurrentControlSet\Services\$name") {
        reg delete "HKLM\SYSTEM\CurrentControlSet\Services\$name" /f 2>&1 | Out-Null
        if (Test-Path "HKLM:\SYSTEM\CurrentControlSet\Services\$name") {
            Write-Host "  FAILED"
        } else {
            Write-Host "  OK"
...
if ($driverInstalled) {
    $sys = "$env:SystemRoot\System32\drivers\$name.sys"
    if (Test-Path $sys) {
        try {
...
# Step 7: Clean build artifacts via MSBuild
Write-Host " [7] Cleaning build artifacts ..."
...
if ($vs) {
    ...
    & $msbuild $proj /p:Configuration=Debug /p:Platform=x64 /t:Clean 2>&1 | Out-Null
    & $msbuild $proj /p:Configuration=Release /p:Platform=x64 /t:Clean 2>&1 | Out-Null
}
Write-Host "  OK"
Write-Host ""

# Step 8: Delete x64 output folder
Write-Host " [8] Deleting x64 output folder ..."
...
if (Test-Path "$env:SystemRoot\System32\drivers\$name.sys") {

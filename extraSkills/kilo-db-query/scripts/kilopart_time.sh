...
# 查询 Kilo 会话 parts（按时间范围过滤）
# 用法: ./kilopart_time.sh <session_id> <start_time> <end_time>
...
sqlite3 -header -column "$KILO_DB" \
  "SELECT id, message_id, datetime(time_created/1000, 'unixepoch', 'localtime') as created, datetime(time_updated/1000, 'unixepoch', 'localtime') as updated, substr(data, 1, 200) as preview FROM part WHERE session_id = '$SESSION_ID' AND time_updated/1000 >= $START_TS AND time_updated/1000 <= $END_TS ORDER BY time_updated ASC;"

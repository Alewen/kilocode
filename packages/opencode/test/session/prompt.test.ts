...
// kilocode_change start - delay post-exit shell output to catch false timeout notes
    ...
    Effect.gen(function* () {
      ...
      return ChildProcessSpawner.make(
        Effect.fnUntraced(function* (command) {
          ...
          const wait = Stream.fromEffect(Effect.sleep("750 millis")).pipe(Stream.flatMap(() => Stream.empty))
...
// kilocode_change start - process exit wins over post-exit output draining
waitingUnix(
  "environment timeout ignores shell output draining after process exit",
  () =>
    withSh(() =>
      withCap(() =>
        provideTmpdirInstance(
          (_dir) =>
            Effect.gen(function* () {
              ...
              expect(tool?.state.output).not.toContain("shell command terminated")
              expect(tool?.state.output).not.toContain("You're running in a sandbox with a fixed timeout.")
            }),
          { git: true, config: cfg },
        ),
      ),

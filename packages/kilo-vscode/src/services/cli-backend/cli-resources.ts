...
export async function copySandboxResources(_source: string, _target: string): Promise<void> {}
...
export function resolveLocalBwrapEnv(
  _extension: string,
  _local: boolean,
  _target = `${process.platform === "win32" ? "win32" : process.platform}-${process.arch}`,
  _root?: string,
): Record<string, string> {
  return {}
}

export async function sanitizeSandboxResources(_bin: string, _local: boolean): Promise<boolean> {
  return false
}
